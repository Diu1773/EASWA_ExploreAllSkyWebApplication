...
for a, b in PAIRS:
    ...
    STOCK = ("공공 천문자료 기반 천문탐구", "공공 천문자료 서비스", "공공 천문자료를 활용한", "실제 자료 기반", "기존 공공 천문자료", "학교 천문탐구", "교육용 웹 플랫폼", "설계 원리", "탐구 흐름", "2022 개정 과학과 교육과정", "현직 교사 중심 현장 검토", "구성하였다. 이를 통해", "제공한다. ESASky는", "현직 교사와 과학교육 전공자", "천문자료를 활용한 학교", "공공 천문자료 기반", "서비스 사례분석, 교육과정",
             # 세 모듈의 이름 나열은 고유명이다. 4장이 「무엇을 골랐나」를,
             # 5장이 「그래서 무엇을 뜻하나」를 말하려면 양쪽 다 이름을 불러야 한다.
             "식현상, 미시중력렌즈, 성단 색등급도")
    def is_stock(w):
        core = w.strip()
...
BAN = {"견주": "비교하다", "겨냥": "다루다·대상으로 하다", "겨누": "대상으로 하다",
       ...
       # 2026-09-13 에 채운 다섯. 문체규칙 1절 표에 있는데 여기 없어 검사를 빠져나갔다.
       # 어간만 적으면 「재현·재분석」·「들어가다」·「가리키다」가 함께 걸린다.
       # 활용형까지 못 박아 그 자리만 잡는다.
       "재려는": "확인하려는·물으려는", "재는 것": "확인하는 것",
       "흔들 수 있": "영향을 줄 수 있", "흔들었": "영향을 주었",
       "움직인다": "변한다·달라진다", "움직였": "변하였",
       "원인을 가리": "원인을 판단하다·구분하다",
       "근거를 들": "근거를 제시하다", "예를 들 수": "예를 제시할 수"}
...
APPENDIX = next((n for n, l in enumerate(L) if l.startswith(CFG.경계("부록시작"))), len(L))
