...
def fig_a():
    ...
    fig, ax = plt.subplots(figsize=(W, 88 * MM))
    ys = range(len(ITEMS))
    h = 0.36
    ax.barh([y - h / 2 for y in ys], [it[2] for it in ITEMS], height=h,
            color=DARK, label="1차 현직 중심 (N=13)")
    ax.barh([y + h / 2 for y in ys], [it[3] for it in ITEMS], height=h,
            color=LIGHT, label="2차 예비교사 (N=13)")
    for y, it in zip(ys, ITEMS):
        ax.text(it[2] + 0.05, y - h / 2, "%.2f" % it[2], va="center",
                fontsize=7.2, color=DARK)
        ax.text(it[3] + 0.05, y + h / 2, "%.2f" % it[3], va="center",
                fontsize=7.2, color=LIGHT)
