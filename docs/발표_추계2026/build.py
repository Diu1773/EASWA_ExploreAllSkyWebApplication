...
cite(sl, "arXiv:2505.19443 · 2606.24429. 2026-09-11 원문 확인. 로고는 simple-icons(CC0), "
         "OpenAI 마크는 그 모음에 있을 때 받아 둔 CC0 판. 상표는 각 소유자의 것임.")

# ═════ 개발 방식 ② 두 방식의 차이 ════════════════════════════════════
sl = S()
y = title(sl, "개발 방식 ② 두 방식의 차이",
          "사람이 매번 묻는 구조에서, 목표를 맡기고 지켜보는 구조로")
pic(sl, "fig_agent_vs.png", M, y + 0.04, W - 2 * M, 4.00, root=HERE, top=True)
bullets(sl, M, y + 4.16, W - 2 * M, [
    ("→ 이 연구가 정한 경계 — 탐구 구조와 단계 설계, 학습자에게 개방할 분석 조건, "
     "천문 모델과 가정은 «연구자가» 정함", ACC, True),
], size=15, gap=9)
cite(sl, "왼쪽 그림 — Sapkota, Roumeliotis and Karkee (2025) 「Vibe Coding vs. Agentic "
...
pic(sl, "fig_verify.png", M, y + 1.25, W - 2 * M, 2.46, root=HERE, top=True)
