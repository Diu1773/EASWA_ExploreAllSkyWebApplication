...
def web_slide(no, head, sub, left, right, tail=None):
    ...
    iy = y + 0.04
    ih = 3.30
    for x, (name, cap1, cap2, cap3) in ((LX, left), (RX, right)):
...
pic(sl, "fig_vibe.png", M, y + 0.02, W - 2 * M, 2.62, root=HERE, top=True)
bullets(sl, M, y + 2.80, W - 2 * M, [
...
pic(sl, "fig_tools.png", M, y + 2.38, W - 2 * M, 1.12, root=HERE, top=True)
bullets(sl, M, y + 3.58, W - 2 * M, [
...
pic(sl, "fig_agent_vs.png", M, y + 0.04, W - 2 * M, 3.62, root=HERE, top=True)
