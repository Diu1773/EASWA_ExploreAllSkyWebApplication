...
순환은 **근거가 결론을 미리 담고 있을 때** 생긴다. 이 원고에서 위험한 자리는 셋이다.
  ...
  A. 초기 설계 방향에서 만든 기준으로 사례를 분석한 뒤, 그 결과에서 같은 설계
     원리를 도출했다고 쓰는 곳. 현재 원고는 분석 기준을 이론과 교육과정·교과서
     검토에서 구체화하고, 사례분석 결과를 두 검토와 함께 최종 원리 정리에 사용한다.
     따라서 세 근거를 함께 밝힌 종합 문장은 순환으로 보지 않는다.
...
RULES = [
    ...
    ("A 사례분석 → 설계 원리",
     ...
     "분석 기준의 출처와 실제 개발 순서를 밝히지 않은 채 사례분석만으로 원리를 도출했다고 쓰면 순환이다."),
    ("B 사용자 응답 → 원리의 타당성",
...
SAFE = r"(아니라|아니다|못 한다|않았다|않는다|한정하여|보아야|자료로 사용|자료로 다루|확인하는 자료|구분하여|이론[^.]{0,100}교육과정|교육과정[^.]{0,100}이론)"
