 ...
 *  [신설] Q18-1 — 문항 15(역문항)의 정문항 짝. 1차에서 역문항 12·15가 하위 1·2위였고 15가
 *         보완 1순위를 만들어냈다. 핵심 지표가 역문항에만 걸려 있으면 실제 난이도인지
 *         역문항 효과인지 가릴 수 없어, 같은 대상을 긍정 진술로 한 번 더 묻는다.
 *
 * 반응척도(문항 8~18) ↔ 표 3-5 11개 + 예비 신설 1개(18-1) = 12개.
 * 역문항 12·15는 위치 비공개, 집계 시 (6 − 점수)로 역채점.
 * 15 ↔ 18-1 은 같은 대상을 반대 방향으로 묻는 짝이므로, 집계 시 두 값을 대조해 보고한다.
...
function createEASWAPreserviceForm() {
  ...
  // ===== 4. EASWA 프로토타입 반응 (공통 11개 + 예비 신설 18-1, 역문항 12·15 — 위치 비공개) =====
  ...
  // 18-1: 문항 15(역문항, "기준값 비교 화면은 …파악하기 어렵다")와 같은 대상을 묻는 정문항 짝.
  // 1차 조사에서 역문항 두 개(12·15)가 하위 1·2위였고, 그중 15가 보완 1순위를 만들어냈다.
  // 핵심 지표가 역문항에만 걸려 있으면 «실제로 어려웠던 것»인지 «역문항이라 낮게 나온 것»인지
  // 가를 수 없다(15는 2점에 7명이 몰린 쏠림이 있었다). 두 값을 대조하기 위해 정문항을 하나 둔다.
  //   같은 방향  → 역문항 효과가 아니라 실제 난이도 (보완 1순위의 근거가 단단해짐)
  //   어긋남     → 역문항 효과였음 (그 자체가 방법론적 발견)
  // 번호를 18-1로 붙인 이유: 뒤 문항을 밀면 역채점 매핑(12·15)과 표 3-5 대응이 어긋난다.
  // 위치를 척도 끝에 둔 이유: 15 바로 옆에 반대말을 놓으면 응답자가 눈치채고 기계적으로 반대로 찍는다.
