...
# ⚠ 2026-09-09 — 아래 현직 값은 **옛것이다.** 13번째 응답을 참가 여부 미결로 빼고 계산한
# N=12 시절 숫자다. 소유자가 13명으로 확정했고 원고 표 9 는 원자료에서 다시 계산한 N=13
# 값으로 이미 고쳐져 있다(8번 4.15 · 9번 4.54 · 11번 4.38 …). **원고가 맞고 이 블록이 틀렸다.**
# 이 스크립트의 현직 열 출력을 원고 근거로 쓰지 않는다. 다시 계산하려면
# ERP2026_Cosmos/논문자료/재계산_표9.py 를 돌린다.
