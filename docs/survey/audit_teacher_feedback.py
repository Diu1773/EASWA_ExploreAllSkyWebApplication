...
def main() -> None:
    ...
    result = dict(status="original_xlsx_verified_author_feedback_integrated_late_response_eligibility_pending",
    ...
    lines = ["# 현직교사 검토 재대조 — 문항 맥락을 보존한 부호화 검토본", "",
             ...
             "> 연구자의 응답별 검토 의견을 통합하여 보정했다. 요약·보정 판정은 AI 작성이며 독립 부호화자 간 신뢰도 확인을 뜻하지 않는다.", "",
    ...
    lines += ["", "범주 폭이 다르므로 이 표의 순위를 그대로 수정 우선순위로 사용하지 않는다. ‘사전 개념·배경’은 용어 풀이만으로 해결되는 요구가 아니다.", "",
              ...
              "## 4. 연구자 검토 후 정한 처리 원칙", "",
              "- 모든 응답을 해석한 뒤 필요한 조치만 개선1~9로 묶었다. 시연 전 우선 후보는 개선1 용어·기본 표기, 개선2 눈에 띄는 어색한 문장, 개선3 단계 안내·이해 점검이다. 전면 문체 정비는 지속 과제다.",
              "- 난이도·차시·선수 지식의 일반 우려는 논의·한계·적용 조건으로 남긴다. 학습 참여, 쉬운 활동, 설명 지원, 교사 자료, 기기, 서버는 구체적으로 필요한 경우 향후 과제에 연결한다.",
              "- 긍정 응답과 수행 답변에 개선을 강제하지 않는다. Q20/R11은 현재 단서의 조건부 장점과 독립 해석의 유보를 함께 보존한다. Q22/R05는 우려 문항 맥락상 개념 난이도 우려다.",
              "- Q6에 대한 연구자의 중간 정정을 앞선 Q6 판단에도 적용했다. Q6/R11 구체 화면 경험만 보충 근거다. 기존 서비스 응답을 EASWA 결함 빈도에 넣지 않는다.",
              "- 원인 서술은 기존 수행 자료이며 새 기능 요구가 아니다. 특정 원인을 말했는지와 실제 원인으로 확인됐는지는 구분한다.",
              "- 퀴즈 팝업·LLM/MCP 도움은 아이디어다. 동시 약 20명 사용 회고, 기존 번역 기능, 과학고·영재 수준 판단은 연구자의 해석·운영 설명과 실제 검증 결과를 구분한다.",
              "- 전체 사용자 원문은 연구자_응답별검토_2026-09-05.md, 최초 AI 판정은 JSON의 initial_judgment에 보존한다. 아래의 연구자 의견은 요약이다.", "",
              ]
    ...
    lines += ["", "## 6. 검증 범위", "",
              ...
              "- 연구자 의견은 44개 응답·66개 의미 단위 모두에 연결했다. 보정 판정의 최종 학술적 확정과 독립 재부호화는 별도다.", ""]
