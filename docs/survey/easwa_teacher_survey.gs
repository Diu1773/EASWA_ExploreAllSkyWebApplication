/*
 * EASWA 프로토타입 반응 및 보완 요구 조사 — 【현장(수업·연수) 대면용】
 * 참여자가 앱을 직접 사용하는 현장 조사 전제. 앱 화면 스크린샷·이미지-only 배려는 제거됨.
 * (온라인 이미지 제시형은 별도의 '온라인용 폼'으로 분리 예정 — 최소 문항, 논문에서는 참고자료로만 사용.)
 * 논문 2026_Cosmos_ERP 3.6(표 3-4·3-5) / 4.5(표 4-8·4-10) 설계 기반. 아래 개편으로 문구·표 조율 필요.
...
function createEASWAForm() {
  ...
  form.setDescription(
    ...
    '\n\n본 조사는 진행자의 안내에 따라 EASWA 주요 단계를 직접 수행해 보신 뒤 응답하는 현장 조사입니다(약 15분). 참여는 자발적이며 언제든 중단할 수 있습니다.'
  );
  ...
  // 온라인 이미지 제시형은 별도의 '온라인용 폼'으로 분리 예정.
  ...
  addScale('13. EASWA의 STEP별 질문과 생각해보기 문항은 각 단계에서 무엇을 확인해야 하는지 이해하는 데 도움이 된다.', '자료 확인·분석 단계의 생각해보기 상자(O/X·선택형 자가점검)를 참고해 주세요.');
  ...
  addScale('16. EASWA는 산출값과 NASA Exoplanet Archive 기준값 또는 문헌값의 차이를 학습자가 스스로 해석하도록 돕는다.',
  ...
  // 현장 폼은 전원 직접 사용 전제 → 이미지-only 진입 분기 불필요(온라인용 폼에서만 사용).
  ...
  //     "직접 수행하지 않음"은 확인방식(Q7) 문항으로 파악되므로 그리드 행에서 제외.
  ...
  // 22: 코호트별(현직/예비) 특이 우려를 서술형으로 포착 — "본인 입장에서" 프레이밍, 필수로 강조.
  //     닫힌 문항의 현직/예비 차이는 Q1(신분)로 집단 비교하므로, 서술형은 이 한 문항에 집중.
  addPara('22. 앞에서 선택하신 본인의 입장(예: 현직 교사 / 예비 교사)에서, EASWA를 실제 수업에 활용하거나 배우는 과정에서 가장 걱정되는 점 한 가지와 그 이유를 적어 주세요.', null, true);
  // 23: 장점·시급 보완·활용 방식(구 22·23·24 통합) — 선택. 논문 표 밖(§4.5 연역 범주 코딩).
  addPara('23. EASWA의 가장 큰 장점과, 보완이 가장 시급한 한 가지, 그리고 수업·교육에서 활용한다면 적절한 방식에 대해 자유롭게 적어 주세요. 기타 의견이 있다면 함께 적어 주세요.');
...
/*
 * 이미지는 createEASWAForm 실행 시 GitHub raw(IMG_BASE)에서 자동으로 삽입됩니다(수동 업로드 불필요).
 * 현장용 폼은 §2 '기존 서비스 4종 실제 화면' 1장(existing_services_2x2.png)만 사용
 * (앱 화면 스크린샷은 현장 직접 사용 전제로 제거됨 — 온라인용 폼에서 다시 사용 예정).
