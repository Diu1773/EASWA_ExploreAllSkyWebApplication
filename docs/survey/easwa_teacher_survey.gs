...
function createEASWAForm() {
  ...
  form.setDescription(
    ...
    '\n\n행사 참여자는 진행자의 안내에 따라 EASWA 주요 단계를 직접 수행한 뒤 응답해 주십시오. 온라인 참여자는 가능한 경우 웹 링크에서 직접 수행해 보신 뒤 응답해 주시고, 여의치 않으면 화면 이미지만으로도 응답하실 수 있습니다(약 15분). 확인 방식은 응답 해석을 위해 구분 수집합니다. 참여는 자발적이며 언제든 중단할 수 있습니다.'
  );
  ...
  addImg('기존 공공 천문자료 서비스 4종 — 실제 화면 (교과서 H-R도 작성 과제 수행)',
    '2022 개정 『지구과학』 교과서의 H-R도 작성 활동을 각 서비스로 실제 수행한 화면입니다. (a) SIMBAD — 천체 정보 검색, (b) VizieR — 카탈로그·표 자료, (c) WorldWide Telescope — 시각화, (d) ESASky — 통합. 각 서비스는 각자의 기능을 신뢰성 있게 수행하지만, 어느 하나도 학교 탐구 흐름 전체(정보 확인 → 표 확보 → 환산·분석 → 시각화 → 해석)를 하나로 연결하지 않아, 과제 완수에는 도구 전환·질의 문법·전문 표기 해독·외부 계산이 필요합니다.',
