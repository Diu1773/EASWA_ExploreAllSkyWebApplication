...
function createEASWAForm() {
  ...
  addScale('10. EASWA는 분석에 사용한 자료의 출처와 관측 정보를 확인할 수 있도록 제시한다.', '화면 3(자료 확인)의 자료 출처·관측 정보를 참고해 주세요.');
  addScale('11. EASWA는 학습자가 분석 조건(측정·분석 설정)을 직접 조정해 볼 수 있는 기능을 제공한다.',
    '예: 이 외계행성 모듈에서는 구경·배경 설정(화면 4·5). 문항은 다른 탐구 모듈에도 적용되도록 일반적으로 서술하였습니다.');
  ...
  form.addCheckboxGridItem()
    ...
    .setHelpText('접속 과정과 화면 1~7의 단계입니다. 해당하는 칸에 모두 체크해 주세요. (본인 = 직접 사용 경험 기준 / 학생 = 예상 기준, 어려운 단계가 없으면 비워 두셔도 됩니다)')
    .setRows([
      '접속·서버 활성화',
      '주제 소개 (화면 1)',
      '대상 선택 (화면 2)',
