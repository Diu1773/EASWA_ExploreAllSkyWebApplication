 ...
 * 정합: 논문 표 3-6과 1:1 (천문학적 타당성 5 · 교육적 활용 가능성 5 · 사용 편의성 3 · 종합 적절성 1 = 14문항, 자유 서술 5, 배경 3).
 *       2026-07-24 점검에서 Q5(주연감광을 산출값으로 오기)·Q12(생각해보기 전 단계 오인) 정정.
 * 대상: 반응 조사 응답자와 역할 구분(논문 3.7) — 7/24 시연 참여 교사는 패널에서 제외할 것.
 * 검토 시점: 두 코호트 반영 보완이 끝난 '최종 보완본'(논문 3.7·4.6). 검토 대상 버전은 docs/DEMO_VERSION_RECORD.md에 기록.
 */
function createEASWAExpertReviewForm() {
  ...
  addCVI('5. 산출값(행성/항성 반지름비 Rp/R*, 궤도 긴반지름비 a/R*, 궤도 경사 등)의 도출 방식이 타당하다. (주연감광 계수는 항성 물성에 따른 표 값을 고정 입력으로 사용하고, 공전 주기는 아카이브 문헌값을 고정합니다.)');
  ...
  addCVI('12. 단계별 질문과 일부 단계(자료 확인·분석 준비)의 생각해보기 문항이 학습자의 탐구를 지원하는 스캐폴딩으로 적절하다.');
