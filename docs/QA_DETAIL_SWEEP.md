...
## 2026-07-24 — 4차 스윕 (야간 자율, 신규 UI 회귀 점검)

- 캡처 25장(1440/1900), fit 자동 생성으로 Step 5·6 포함 전 화면. 프로덕션 번들(index-CfIOnIGo)에
  이번 주 변경 7종(개념도·색힌트·아카이브 표기·유도·사이트피드백·CTA·용어집) 전부 반영 확인.
- 점검 초점: Step 0 파이프라인 개념도, Step 5 아카이브 박스·유도, Step 6 피드백 위치 — 각 1900 폭.
- **신규 발견 0건 — 수렴 선언.** (직전 3차의 자동수정·보고 항목 재발 없음)
- 부수 발견(스윕 외): docs/survey/qr_form.png가 옛 폼(1FAIpQLSeiB2u…)을 가리키고 있었음 →
  신규 폼으로 재생성·디코드 왕복 검증(커밋 0db610a). 인쇄물 QR 재확인 필요는 체크리스트에 기록.
